    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        switch (mState) {
            case StateNormal:
                ...
                break;
            case StateTouchDown:
                if (mColor == 0x00000000 || mColor == 0xffffff)
                    ripplePaint.setAlpha(60);
                else
            ...
            case StateTouchUp:
                if (elapsed < ANIMATION_DURATION) {
                    int alpha = 0;
                    if (mColor == 0x00000000 || mColor == 0xffffff)
                        alpha = Math.round((ANIMATION_DURATION - elapsed) * 60 / ANIMATION_DURATION);
                    else
                        alpha = Math.round((ANIMATION_DURATION - elapsed) * 255 / ANIMATION_DURATION);
